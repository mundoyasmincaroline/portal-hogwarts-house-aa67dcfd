...
-- Create house enum
DO $$ BEGIN CREATE TYPE public.house_type AS ENUM ('gryffindor', 'slytherin', 'ravenclaw', 'hufflepuff'); EXCEPTION WHEN duplicate_object THEN null; END $$;
...
-- user_challenges
DROP POLICY IF EXISTS "Authenticated can view user_challenges" ON public.user_challenges;
...
-- house_points
DROP POLICY IF EXISTS "Authenticated can view house_points" ON public.house_points;
CREATE POLICY "Authenticated can view house_points" ON public.house_points FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "Admins can manage house_points" ON public.house_points;
...
-- user_badges
DROP POLICY IF EXISTS "Authenticated can view user_badges" ON public.user_badges;
...
DROP POLICY IF EXISTS "Authenticated can view reactions" ON public.post_reactions;
...
DROP POLICY IF EXISTS "Admins can insert moderation log" ON public.moderation_log;
...
DROP POLICY IF EXISTS "Canais visíveis para admin" ON public.channels;
CREATE POLICY "Canais visíveis para admin" ON public.channels FOR SELECT
    USING (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "Canais gerais visíveis para todos" ON public.channels;
...
DROP POLICY IF EXISTS "Canais de casas visíveis para membros da casa" ON public.channels;
CREATE POLICY "Canais de casas visíveis para membros da casa" ON public.channels FOR SELECT
    USING (
        allowed_houses IS NOT NULL 
        AND (SELECT house::text FROM profiles WHERE user_id = auth.uid()) = ANY(allowed_houses)
    );

-- Políticas de Mensagens
DROP POLICY IF EXISTS "Mensagens visíveis se tem acesso ao canal" ON public.messages;
    ...
    USING (
        EXISTS (
            ...
            WHERE c.id = channel_id AND (
                ...
                OR (c.allowed_houses IS NOT NULL AND (SELECT house::text FROM profiles WHERE user_id = auth.uid()) = ANY(c.allowed_houses))
            )
        )
    );
...
-- Políticas
DROP POLICY IF EXISTS "Insta posts visíveis para todos" ON public.insta_posts;
CREATE POLICY "Insta posts visíveis para todos" ON public.insta_posts FOR SELECT
    USING (true);

DROP POLICY IF EXISTS "Usuários podem inserir insta posts" ON public.insta_posts;
CREATE POLICY "Usuários podem inserir insta posts" ON public.insta_posts FOR INSERT
    WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Usuários podem atualizar likes em insta posts" ON public.insta_posts;
CREATE POLICY "Usuários podem atualizar likes em insta posts" ON public.insta_posts FOR UPDATE
    USING (true);

DROP POLICY IF EXISTS "Usuários podem deletar seus próprios insta posts" ON public.insta_posts;
...
-- Políticas
DROP POLICY IF EXISTS "Qualquer um pode ler palavras proibidas" ON public.banned_words;
CREATE POLICY "Qualquer um pode ler palavras proibidas" ON public.banned_words FOR SELECT USING (true);
DROP POLICY IF EXISTS "Apenas admin pode gerenciar palavras proibidas" ON public.banned_words;
...
DROP POLICY IF EXISTS "Anyone can view characters_birthdays" ON public.characters_birthdays;
...
DROP POLICY IF EXISTS "Users can update own cooldowns" ON public.user_cooldowns;
CREATE POLICY "Users can update own cooldowns" ON public.user_cooldowns FOR UPDATE TO authenticated USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Users can insert own cooldowns" ON public.user_cooldowns;
...
DROP POLICY IF EXISTS "Anyone can view enigmas" ON public.enigmas;
...
DROP POLICY IF EXISTS "Anyone can view global_events" ON public.global_events;
