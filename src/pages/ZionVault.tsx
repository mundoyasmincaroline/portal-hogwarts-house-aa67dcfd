...
const ZionVault: React.FC = () => {
  ...
  const verifyPasscode = () => {
    // Código de acesso de emergência (demonstração)
    if (passcode === "ZION2026" || profile?.username === 'morpheus') {
      setAccessGranted(true);
      toast.success("ACESSO CONCEDIDO - PROTOCOLO ZION ATIVADO", {
        style: { background: '#065f46', color: '#34d399', border: '1px solid #34d399' }
      });
    } else {
  ...
  if (!isFamily) {
    return (
      <div className="min-h-screen bg-[#050505] flex items-center justify-center p-6 font-mono">
        <div className="max-w-md w-full p-8 border-2 border-red-900 bg-red-950/10 rounded-lg shadow-[0_0_50px_rgba(153,27,27,0.2)] text-center space-y-6">
          ...
          <p className="text-red-900 text-sm italic">
            "Se você não é da linhagem, este lugar não existe para você."
          </p>
        </div>
      </div>
    );
  ...
  if (!accessGranted) {
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center p-6 font-mono relative overflow-hidden">
        ...
        <div className="max-w-sm w-full glass-dark p-10 rounded-3xl border border-emerald-500/20 relative z-10 space-y-8 shadow-[0_0_100px_rgba(16,185,129,0.1)]">
          <div className="text-center">
            ...
            <p className="text-emerald-900 text-[10px] mt-2">AUTENTICAÇÃO NECESSÁRIA</p>
          </div>
          <div className="space-y-4">
             <input 
              ...
              className="w-full bg-black/80 border border-emerald-900/50 rounded-xl px-4 py-4 text-emerald-500 placeholder-emerald-900 text-center tracking-[1em] outline-none focus:border-emerald-500 transition-all shadow-inner"
             />
             <Button className="w-full h-14 bg-emerald-600 hover:bg-emerald-500 text-black font-bold tracking-widest rounded-xl" onClick={verifyPasscode}>
                ENTER ZION
             </Button>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-emerald-500 font-mono p-4 md:p-10 relative overflow-hidden selection:bg-emerald-500 selection:text-black">
      ...
      <div className="relative z-10 max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-8 space-y-6">
            ...
            <div className="glass-dark border-emerald-900/30 p-8 rounded-none border-l-4 border-l-emerald-500 bg-emerald-950/5">
               <h3 className="text-xs font-bold uppercase tracking-[0.4em] mb-6 flex items-center justify-between">
                  <span className="flex items-center gap-3"><HardDrive size={18} /> Zion Portal Engine — Core Status</span>
               ...
               <div className="space-y-4">
                  <div className="p-4 bg-black/60 border border-emerald-900/20 flex flex-col gap-3">
                     <div className="flex justify-between text-[10px] font-bold">
                        <span className="text-emerald-500 tracking-widest">PORTAL LOAD (CAULDRON CAPACITY)</span>
                  ...
                  <div className="p-4 bg-black/60 border border-emerald-900/20 flex flex-col gap-3">
                     <div className="flex justify-between text-[10px] font-bold">
                        <span className="text-emerald-500 tracking-widest">DATABASE SYNC (GRINGOTTS LEDGER)</span>
               ...
               <div className="mt-8 space-y-2">
                  <p className="text-[10px] text-emerald-900 uppercase font-bold mb-4 tracking-widest">Arquivos Críticos de Infraestrutura</p>
                  {[
                    { name: "Portal_Core_v4.system", size: "4.2GB", date: "22/04/2026" },
                    { name: "Database_Mapping_Gringotts.db", size: "890MB", date: "21/04/2026" },
                    { name: "RLS_Security_Policies.shield", size: "12KB", date: "22/04/2026" },
                  ].map((file, i) => (
                    <div key={i} className="flex items-center justify-between p-3 bg-black/20 border border-emerald-900/10 hover:border-emerald-500/30 transition-all cursor-pointer group">
                       <div className="flex items-center gap-3">
                          <Terminal size={14} className="text-emerald-700" />
                          <span className="text-xs text-emerald-300">{file.name}</span>
                       </div>
                       <span className="text-[9px] text-emerald-900">{file.size}</span>
                    </div>
                  ))}
               </div>
            </div>

            <div className="glass-dark border-emerald-900/20 p-8 rounded-none bg-black/40 h-[300px] flex flex-col">
               <h3 className="text-xs font-bold uppercase tracking-[0.4em] mb-4 flex items-center gap-3">
                  <Activity size={18} /> Security Logs
               </h3>
               <div className="flex-1 overflow-y-auto space-y-1 font-mono text-[11px] text-emerald-900 custom-scrollbar">
                  <p className="text-emerald-500 animate-pulse">[OK] Zion Mainframe Initialized...</p>
                  <p>[LOG] Passcode verified for user: {profile?.username}</p>
                  <p>[LOG] Backup completed for 'Yasmin_World' database.</p>
                  <p>[WARN] Attempt to access Zion from unauthorized node: BLOCKED</p>
                  <p className="text-emerald-500 animate-pulse">[OK] All protocols green.</p>
                  {Array.from({length: 10}).map((_, i) => (
                    <p key={i}>[TRACE] Packet {Math.random().toString(16).slice(2, 10)} verified and routed to ZionCore.</p>
                  ))}
               </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-4 space-y-8">
            <div className="glass-dark border-emerald-500/20 p-8 rounded-none bg-emerald-950/10 relative overflow-hidden group">
               <div className="absolute top-0 right-0 p-4 opacity-20">
                  <Zap size={60} className="text-emerald-500" />
               </div>
               <h4 className="text-[10px] font-bold uppercase tracking-[0.3em] mb-6">Defensive Core</h4>
               <div className="space-y-6">
                  <div>
                     <div className="flex justify-between text-[9px] mb-2 font-bold tracking-widest">
                        <span>FIREWALL STRENGTH</span>
                        <span>99.9%</span>
                     </div>
                     <div className="h-1 bg-emerald-950 w-full"><div className="h-full bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.8)]" style={{width: '99.9%'}} /></div>
                  </div>
                  <div>
                     <div className="flex justify-between text-[9px] mb-2 font-bold tracking-widest">
                     ...
                     <div className="h-1 bg-emerald-950 w-full"><div className="h-full bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.8)]" style={{width: '100%'}} /></div>
                  </div>
               </div>
            </div>

            <div className="glass-dark border-emerald-900/30 p-8 rounded-none space-y-6">
               <div className="flex items-center gap-4 text-emerald-500">
                  <EyeOff size={24} />
                  <p className="text-xs font-bold uppercase tracking-widest leading-relaxed">
                     O portal agora é invisível para radares não-autorizados.
                  </p>
               </div>
               <p className="text-[10px] text-emerald-900 leading-relaxed italic">
                 "Em Zion, não apenas protegemos os dados. Protegemos a memória de quem somos e de quem amamos."
               </p>
            </div>
          </div>
        </div>
