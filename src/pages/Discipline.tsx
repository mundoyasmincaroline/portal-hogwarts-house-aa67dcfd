...
export default function Discipline() {
  ...
  const load = async () => {
    ...
    const [d, m, am] = await Promise.all([
      ...
      supabase.from("merits").select("*, profile:profiles!merits_user_id_fkey(username,house)").order("created_at", { ascending: false }).limit(20),
    ]);
          ...
          {allMerits.map(m => (
            <div key={m.id} className="glass-premium rounded-xl p-3 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm truncate">{m.profile?.username ?? 'Bruxo'} <span className="text-[10px] text-muted-foreground capitalize">· {m.profile?.house}</span></p>
